...
def main():
    f_out = open("/home/administrator/aopt/logs/aopt_delete_address.log", "a+")

    # Parsing argurments
    parser = OptionParser()

    parser.add_option("-n", "--number", dest="number_list", type="string", help="Number")

    (options, args) = parser.parse_args()

    number_list = options.number_list

    print "*** Huy / Thanh ly dau so ***"
    if " " in number_list:
    ...
    if num_max == 0:
        ...
        for n in range(num_max):
            ...
            if ((len(number) == 11) and (number[0:2] == "02")) or ((len(number) == 8) and (number[0:1] == "1")) or (
                    (len(number) == 10) and (number[0:1] == "1")) or ((len(number) == 10) and (number[0:2] in ("09","08","07","03"))):
                time_now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                out_put = ""
