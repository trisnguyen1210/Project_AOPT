...
def create_rule(ruleset, number):
    xmlData = '''<daml command="create"><rule><name>''' + number + '''</name><priority>1</priority><execTime>POSTROUTING</execTime><destPattern>''' + number + '''</destPattern><rejectCode>403</rejectCode><action>next</action><rejectPhrase>Forbidden</rejectPhrase><prefIdPattern>(.*)</prefIdPattern><sourcePresentationReplace>SHOW</sourcePresentationReplace><ruleset>''' + ruleset + '''</ruleset></rule></daml>'''  # Xem trong AS.6.9 What news
    payload = {"XmlData": xmlData, "UserRequest": "AOPT"}
    time.sleep(1)
    result = requests.post(url_write_account, data=payload)
    response = result.text
...
def main():
    global f_out
    f_out = open("/home/administrator/aopt/logs/aopt_block_incoming.log", "a+")

    parser = OptionParser()

    parser.add_option("-k", "--number_list", dest="number_list", type="string", help="Input number")
    parser.add_option("-n", "--name", dest="name", type="string", help="Input name KHG")

    (options, args) = parser.parse_args()

    if (options.number_list == "None" or  options.name == "None"):
        print 'Input Contract Error'
    for op in ("number_list", "name"):
    ...
    f_out.write(timenow() + " # Start # Block Incoming: " + number_list + "," + name + "\n")
    ...
    if num_max == 0:
        ...
        for n in range(num_max):
            ...
            account = read_address(number)
            if account != "":
                ruleset = "Block : Incoming " + name + " " + account
                ...
                if get_ruleset_all != "" and get_ruleset_all != "Fail":
                    print "Account has ruleset "       # Account co cau hinh Ruleset khac
                    if ruleset not in get_ruleset_all: # neu ruleset block incoming ko co trong account
                        print "Ruleset " + ruleset + " has not been added to account\n"
                        if ruleset_find not in find_rulset_aar: # neu ruleset block incoming ko co trong AAR
                            print "Ruleset " + ruleset + " has not been created"
                            create_ruleset(ruleset)
                            print "Create ruleset " + ruleset + " OK"
                            create_rule(ruleset, number)
                            print "Rule ", number, " has been created successfully"
                            ruleset_add = get_ruleset_all + "<ruleset>" + ruleset + "</ruleset>"
                            f_out.write(timenow() + " Ruleset: " + ruleset + " has been created\n")
                        else: # neu ruleset block incoming co trong AAR
                            print "Ruleset " + ruleset + " was created before"
                            if number in find_rulset_aar:  # neu rule da ton tai trong ruleset
                                print "Rule " + number + " was configured"
                                #ruleset_add = get_ruleset_all + "<ruleset>" + ruleset + "</ruleset>"
                            else:
                                #print "Rule " + number + " has not been configured"
                                create_rule(ruleset, number)
                                print "Rule ", number, " has been created successfully"
                            ruleset_add = get_ruleset_all + "<ruleset>" + ruleset + "</ruleset>"
                    else:   # neu ruleset block incoming co trong account
                        print "Ruleset " + ruleset + " was added to account before"
                        if number in find_rulset_aar:
                            print "Rule ", number, " was configured before"
                            ruleset_add = "Fail"
                        else:
                            print "Rule ", number, " has not been configured before"
                            create_rule(ruleset, number)
                            print "Rule ", number, " has been added to account successfully"
                            ruleset_add = "OK"
    ...
    f_out.write(timenow() + " # End # Block spam call: " + number_list + "," + name + "\n")
