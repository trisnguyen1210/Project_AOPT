...
import aopt_add_ip_bk_trien_khai_moi

url_read_account = 'http://172.27.16.140:8888/daml/account/read_test'
url_write_account = 'http://172.27.16.140:8888/daml/account/write_test'
url_read_address = 'http://172.27.16.140:8888/daml/address/read_test'
url_write_address = 'http://172.27.16.140:8888/daml/address/write_test'
url_write_address_hcm = 'http://172.27.16.140:8888/daml/address/write/hcm'
url_read_siptrunk = 'http://172.27.16.140:8888/daml/siptrunk/read_test'
url_write_siptrunk = 'http://172.27.16.140:8888/daml/siptrunk/write_test'
url_read_ruleset = 'http://172.27.16.140:8888/daml/ruleset/read_test'
url_write_ruleset = 'http://172.27.16.140:8888/daml/ruleset/write_test'
...
def run(value, payload):
    ...
    #time.sleep(1)
    result = requests.post('http://172.27.16.140:8888/daml/' + value + '/write_test', data=payload)
...
def get_type_number(number):
    if number[0:3] in ("090","093","070","076","077","078","079","089"):
        type_number = "Mobifone"
    elif number[0:3] in ("091","094","081","082","083","084","085","088"):
        type_number = "Vinaphone"   
    elif number[0:3] in ("096","097","098","032","033","034","035","036","037","038","039","086"):
        type_number = "Viettel"
    else:
        type_number = ""
    return type_number
...
def check_add(number):
    ...
    try:
        ...
        if "ok" in response_hcm and "ok" in response_hni:
            ...
            if number in response_hcm or number in response_hni: # dau so da ton tai
                #print "True"
                get_account_add_hcm = response_hcm[response_hcm.find("<account>") + 9:response_hcm.find("</account>")]
                get_account_add_hni = response_hcm[response_hni.find("<account>") + 9:response_hni.find("</account>")]
                return True
...
def check_value_acc(account):
    ...
    response_hni = split_response(response)[1]
    if "ok" in response_hcm and "ok" in response_hni:
        get_value_max = response_hcm[response_hcm.find("<valueMax>") + 10:response_hcm.find("</valueMax>")]
    return get_value_max
def add_value_acc(account,valueMax):
    headers = {'content-type': 'application/json'}
    xmlData = '''<daml command="write"><account><accountName>'''+account+'''</accountName><sysAccountTopStop><valueMax>'''+str(valueMax)+'''</valueMax></sysAccountTopStop></account></daml>'''
    payload = {"XmlData": xmlData, "UserRequest": "AOPT"}
    ...
    response_hni = split_response(response)[1]
    if "status=ok" in response_hcm and "status=ok" in response_hni:
        print "DAML Add value OK"
        return True
    else:
        print "DAML Add value NOK"
        return False
...
def check_map_partner(account):
    ...
    response_hcm = split_response(response)[0]
    response_hni = split_response(response)[1]
    tenantCheck = response_hcm[response_hcm.find("<tenant>") + 8:response_hcm.find("</tenant>")]
    print(account)
    split= tenantCheck.split('-')[2]
    return split
...
def main():
    f_out = open("/home/administrator/aopt/logs//aopt_add_khg_new_mobile_trunking.log", "a+")
    #f_out = open("/home/administrator/ductt7/aopt-test/aopt_add_khg_inside_new/aopt_add_khg_inside_new.log", "a+")
    f_list = open("/home/administrator/aopt/logs//aopt_add_khg_new_mobile_trunking.csv", "a+")
    global c_list
    c_list = open("/home/administrator/aopt/logs//update_acc_inside_fail.csv", "a+")
    parser = OptionParser()

    # parser.add_option("--pc", "--parentshd", dest="shd", type="string", help="So Hop Dong cha")
    parser.add_option("-c", "--shd", dest="shd", type="string", help="So Hop Dong")
    ...
    parser.add_option("--lp", dest="list_phone_number", type="string", help="List phone number")
    parser.add_option("--partner", dest="partner", type="string", help="Doi tac Mobile trunking: Telesales, ViHAT, SouthTelecom, STEL, VNTEL, ITS")
    ...
    if (options.type_KHG == "SIP Trunk") or (options.type_KHG == "SIP TRUNK"):
        for option in ('shd', 'name_KHG', 'number_list', 'ip_KHG', 'port_KHG' ,'partner'):
        ...
        for option in ('shd', 'name_KHG', 'number_list', 'valueMax','partner'):
            if not getattr(options, option):
                print 'Gia tri %s chua duoc nhap vao' % option
                parser.print_help()
                sys.exit()
    ...
    if (check_input(shd,name_KHG,type_KHG,number_list,valueMax,ip_KHG,port_KHG,host_IP,ip_KHG_BK1,port_bk1,host_IP_BK1,ip_KHG_BK2,port_bk2,host_IP_BK2)==True):
        ...
        print "*** Cau hinh moi KHG Mobile Trunking ***"
        print "Input:"
        if type_KHG.upper() == "SIP TRUNK":
            print "Hop dong:", shd
            # print "Hop dong con:",shd
            print "Ten KHG:", name_KHG
            print "Number:", number_list
            print "Loai dich vu:", type_KHG
            print "IP Main: " + str(ip_KHG) + ", port IP main: " + str(port_KHG) + ", host IP main: " + host_IP
            print "IP BK1: " + str(ip_KHG_BK1) + ", port IP BK1: " + str(port_bk1) + ", host IP BK1: " + host_IP_BK1
            print "IP BK2: " + str(ip_KHG_BK2) + ", port IP BK2: " + str(port_bk2) + ", host IP BK2: " + host_IP_BK2
            print "Han muc su dung cua dau so:", valueMax
            # print "Han muc su dung cua HD cha:",parentValueMax
            print "Doi tac:", partner
        else:
            print "Hop dong:", shd
            # print "Hop dong con:",shd
            print "Ten KHG:", name_KHG
        ...
        print
        print "Checking..."
        f_out.write(timenow() + " # Start # Cau hinh moi KHG: " + shd + "," + name_KHG + "," + number_list + "," + type_KHG + "," + ip_KHG + "," + port_KHG + "," + host_IP + "," + ip_KHG_BK1 + "," + port_bk1 + "," + host_IP_BK1 + "," + ip_KHG_BK2 + "," + port_bk2 + "," + host_IP_BK2 + "," + valueMax + "\n")
        if type_KHG.upper() == "SIP TRUNK":
            f_out.write(
        ...
        print "Trien khai tong cong: " + str(num_max) + " dau so"
        for n in range(num_max):
            ...
            if status_check_add == True: # neu dau so ton tai
                print "Number: ", number, ' --> Da ton tai tren AAR'
                #print "Number: ", number, ' --> Da ton tai tren AAR-HNI voi account ', get_account_add_hni
                f_out.write(timenow() + " " + number + ' da ton tai tren AAR' + "\n")
                #f_out.write(timenow() + " " + number + ' da ton tai tren AAR-HNI voi account ' + str(get_account_add_hni) + "\n")
                print "Output: Fail"
                list_fail.append(number)
            elif ((number[0:1]!="01") and (number[0:1]!="02")): # neu dau so chua ton tai va khac so co dinh, so di dong cu
                if get_type_number(number) == "":
                    break
                else:
                ...
                for x in range(len(temp_list)): # Tim account cung cau hinh, neu cung cau hinh thi add vao account do, khong co thi tao account khac
                    acc = temp_list[x]
                    print "\nCheck account", acc
                    ...
                    if status == "OK": # neu account ton tai va khong bi disable thi tim account cung cau hinh va add vao
                        if type_number == "1900" or type_number == "1800":
                            ...
                            blocked = "false"
                        time_now = timenow()
                        check_map_partner1= check_map_partner(acc)
                        if type_KHG.upper() == "SIP TRUNK":
                            list_newsiptrunk = []
                            ...
                            if status_check_sipacc_siptrunk == "SIP Trunk":  # neu account la SIP Trunk
                                ...
                                if check_map_siptrunk_account(acc, list_newsiptrunk) and check_map_partner1 == partner:  # neu account cau hinh giong trunk IP
                                    ...
                                    if add_address_account(acc, number, blocked, time_now, type_KHG):
                                        ...
                                        checkvalue=int(float(check_value_acc(acc)))
                                        value1=(checkvalue)+int(float(valueMax))
                                        add_value_acc(acc,value1)
                                        done = True
                                        f_out.write(timenow() + " Number: " + number + " has been added to account " + acc + "\n")
                                        print "Output: OK"

                                        if ((ip_KHG_BK1 != "") and (host_IP_BK1 != "")):  # neu co IP BK1 thi add them vao account
                                            print "Adding IP Backup 1"
                                            account = acc
                                            ip_bk = ip_KHG_BK1
                                            port_bk = port_bk1
                                            host_ip_bk = host_IP_BK1
                                            aopt_add_ip_bk_trien_khai_moi.add_ip_bk_to_account(account, name_KHG, ip_bk, port_bk, host_ip_bk)
                                        if ((ip_KHG_BK2 != "") and (host_IP_BK2 != "")):  # neu co IP BK2 thi add them vao account
                    ...
                    while x < 100:
                        ...
                        if new_account in acc_list:
                            ...
                            if type_KHG.upper() == "SIP TRUNK": # neu yeu cau trien khai SIP Trunk
                                ...
                                checksiptrunk = check_siptrunk(ip_KHG, port_KHG) # kiem tra IP va port da ton tai tren he thong chua
                                if checksiptrunk == False: # neu ip va port da ton tai thi kiem tra host va loai ket noi
                                    ...
                                    type = "Primary"
                                    list_new_sipTrunkName = create_sipTrunkName(shd, ip_KHG, port_KHG, type, host_IP)  # Tao SIP Trunk Name IP Primary moi de check
                                    ...
                                    list_new_sipTrunkName = create_sipTrunkName(shd, ip_KHG, port_KHG, type,host_IP)
                                    list_new_sipTrunkInfo = create_sipTrunkInfo(shd, name_KHG, ip_KHG, port_KHG, "Primary", host_IP)
                                    xmlDataSIPTrunk1 = xmlSipTrunk(list_new_sipTrunkName[0], list_new_sipTrunkInfo[0], ip_KHG, port_KHG, host_IP, sbc_main, weight_main)
                                    xmlDataSIPTrunk2 = xmlSipTrunk(list_new_sipTrunkName[1], list_new_sipTrunkInfo[1], ip_KHG, port_KHG, host_IP, sbc_backup, weight_bk)
                                ...
                                if ip_KHG_BK1 == "":
                                    info = "SIP MobileTrunking to " + partner + " Account " + name_KHG + " " + new_account + " IP-" + ip_KHG + "_" + host_IP
                                elif ((ip_KHG_BK1 != "") and (host_IP_BK1 != "")):
                                    info = "SIP MobileTrunking to " + partner + " Account " + name_KHG + " " + new_account + " IP-" + ip_KHG + "_" + host_IP + " BK-" + ip_KHG_BK1 + "_" + host_IP_BK1

                            elif type_KHG.upper() == "SIP ACCOUNT": # neu yeu cau trien khai SIP Account
                                username = number
                                info = "SIP Account Register MobileTrunking to " + partner + " - " + name_KHG + " " + new_account
                            ...
                            if stop == False:
                                ...
                                if type_number == "1800":
                                    ...
                                    tenant = "FPT-MobileTrunk-"+partner
                                ...
                                elif (shd == "HNMB00132"): #APAX
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to SIP MobileTrunk ViHAT FOR HNMB00132"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                    if (type_number == "Viettel"):
                                        pricelist = "MobileTrunking-"+partner+"-Viettel"
                                    elif (type_number == "Mobifone"):
                                        pricelist = "MobileTrunking-"+partner+"-Mobi"
                                    elif (type_number == "Vinaphone"):
                                        pricelist = "MobileTrunking-"+partner+"-Vina"
                                elif (shd == "HNMB00138"): #VPBANK
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to SIP MobileTrunk ViHAT FOR VPbank"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                    if (type_number == "Viettel"):
                                ...
                                elif (type_number == "Viettel") and partner == "SouthTelecom" : # SouthTelecom Viettel
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to Southtelecom For MobileTrunking"
                                    pricelist = "MobileTrunking-SOUTH-Viettel"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                elif (type_number == "Mobifone") and partner == "SouthTelecom" : # SouthTelecom Mobi
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to Southtelecom For MobileTrunking"
                                    pricelist = "MobileTrunking-SOUTH-Mobi "
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Mobifone</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                elif (type_number == "Vinaphone") and partner == "SouthTelecom" : # SouthTelecom Vina
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to Southtelecom For MobileTrunking"
                                    pricelist = "MobileTrunking-SOUTH-Vina"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Vinaphone</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"

                                elif partner == "ITS" : # ITS Viettel
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to ITS For MobileTrunking"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                    if (type_number == "Viettel"):
                                        pricelist = "MobileTrunking-ITS-Viettel"
                                        ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    elif (type_number == "Mobifone"):
                                        pricelist = "MobileTrunking-ITS-Mobi"
                                        ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Mobifone</ruleset>"
                                    elif (type_number == "Vinaphone"):
                                        pricelist = "MobileTrunking-ITS-Vina"
                                        ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Vinaphone</ruleset>"
                                    else:
                                        return error

                                elif (type_number == "Viettel") and partner == "STEL" : # STEL Viettel
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to STEL For MobileTrunking"
                                    pricelist = "MobileTrunking-STEL-Viettel"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                elif (type_number == "Mobifone") and partner == "STEL" : # STEL Mobi
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to STEL For MobileTrunking"
                                    pricelist = "MobileTrunking-STEL-Mobi "
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Mobifone</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                elif (type_number == "Vinaphone") and partner == "STEL" : # STEL Vina
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to STEL For MobileTrunking"
                                    pricelist = "MobileTrunking-STEL-Vina"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Vinaphone</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"

                                elif (type_number == "Viettel") and partner == "VNTEL" : # VNTEL Viettel
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to VNTEL For MobileTrunking"
                                    pricelist = "MobileTrunking-VNTEL-Viettel"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                elif (type_number == "Mobifone") and partner == "VNTEL" : # VNTEL Mobi
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to VNTEL For MobileTrunking"
                                    pricelist = "MobileTrunking-VNTEL-Mobi "
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Mobifone</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                elif (type_number == "Vinaphone") and partner == "VNTEL" : # VNTEL Vina
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to VNTEL For MobileTrunking"
                                    pricelist = "MobileTrunking-VNTEL-Vina"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Vinaphone</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"

                                elif (type_number == "Viettel"):
                                    sysAccountTopStop = '''<valueMax>''' + valueMax + '''</valueMax><alarmLevel>0.8</alarmLevel><alarmEmail>FTEL.FTI.VOIP@FPT.COM.VN,ivoice.support@fpt.com.vn,FTEL.FTI.VOICE@FPT.COM.VN,ftel.ftihn.voice@FPT.COM.VN</alarmEmail><dailyMax>0.0</dailyMax><monthlyReset>true</monthlyReset><dailyReset>false</dailyReset>'''
                                    routingTable = "Route to SIP Mobile Trunking " + partner
                                    pricelist = "MobileTrunking-"+partner+"-Viettel"
                                    ruleset = "<ruleset>Block : All Outgoing International Calls</ruleset><ruleset>Block : International Call - High Cost</ruleset><ruleset>Block : On-net for Mobile Trunking</ruleset><ruleset>Block : Outgoing External Calls Mobile Trunking for Viettel</ruleset>"
                                    blocked = "false"
                                    maliciousCallerId = "true"
                                ...
                                if add_new_account(new_account, info, tenant, routingTable, pricelist, username, password, ruleset, time_now, maliciousCallerId, sysAccountTopStop):
                                    ...
                                    if add_address_account(new_account, number, blocked, time_now, type_KHG):
                                        ...
                                        f_out.write(timenow() + " Number: " + number + " has been aldded to account " + new_account + "\n")
                                        if type_KHG.upper() == "SIP TRUNK": # neu yeu cau la SIP Trunk thi add trunk vao account
                                            ...
                                            for y in range(len(list_newsiptrunk)):
                                                if map_siptrunk_account(new_account, list_newsiptrunk[y]):
                                                    print "Map trunk IP " + list_newsiptrunk[y] + " to " + new_account + " OK"
                                                    print "Output: OK"
                                                    f_out.write(timenow() + " IP Trunk: " + list_newsiptrunk[y] + " has been added to account " + new_account + "\n")
                                                else:
                                                    print "Map trunk IP " + list_newsiptrunk[y] + " to " + new_account + " NOK"
                                                    if number not in list_fail:
                                                        list_fail.append(number)
                                                    print "Output: Fail"
                                            if ((ip_KHG_BK1 != "") and (host_IP_BK1 != "")):  # neu co IP BK1 thi add them vao account
                                        ...
                                        #     f_out.write(timenow() + " Cap nhat Inside: " + shd + ", " + new_account + " OK" + '\n')
                                        #     print "Output: OK"
                                        # else:
                                        #     f_out.write(timenow() + " Cap nhat Inside: " + shd + ", " + new_account + " NOK" + '\n')
        ...
        if list_fail != []:
            ...
            print "Output: OK"
        f_out.write(timenow() + " # End # Cau hinh moi KHG: " + shd + "," + name_KHG + "," + number_list + "," + type_KHG + "," + ip_KHG + "," + port_KHG + "," + host_IP + "," + ip_KHG_BK1 + "," + port_bk1 + "," + host_IP_BK1 + "," + ip_KHG_BK2 + "," + port_bk2 + "," + host_IP_BK2 + "," + valueMax + "\n")
        f_out.close()
